...
export function weekdayKey(dt: DateTime): keyof OpeningHoursMap {
  return WEEK_KEYS[dt.weekday % 7] as keyof OpeningHoursMap; // sun=0 with %7 trick
}

export function parseRange(dayISO: string, tz: string, range: string): { start: DateTime; end: DateTime } {
...
export function openingWindowsForDate(dayISO: string, tz: string, opening: OpeningHoursMap, special?: SpecialDayConfig) {
  const dt = DateTime.fromISO(dayISO, { zone: tz });
  if (!dt.isValid) return [];
  const override = special?.[dayISO];
  const ranges = Array.isArray(override) ? override
               : override === null ? [] // forced closed
               : (opening[weekdayKey(dt)] ?? []);
  const windows = ranges.map(r => parseRange(dayISO, tz, r)).filter(w => w.start.isValid && w.end.isValid && w.start < w.end);
  return windows;
}

/** Build a slot grid (start-inclusive, end-exclusive) with given slot minutes, confined inside opening windows. */
export function buildSlotsForDay(dayISO: string, tz: string, opening: OpeningHoursMap, slotSizeMinutes: number, special?: SpecialDayConfig): Interval[] {
